...
public class HumanInput : PlayerInput {
	...
	public PlayerInput.InputData MoveData()
	{
		...
		if (targetPosition.HasValue) {
			...
			data.mouseX = -4 * Vector3.SignedAngle ((Vector3)targetPosition - cameraSlider.position, cameraSlider.forward, Vector3.up) * sensitivityy;

			data.mouseY = 4 * Vector3.SignedAngle ((Vector3)targetPosition - cameraSlider.position, cameraSlider.forward, Vector3.Cross (cameraSlider.forward, Vector3.up)) * sensitivityy;
